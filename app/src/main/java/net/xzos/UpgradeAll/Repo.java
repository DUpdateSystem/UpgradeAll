...
class Repo {
    static void refreshData() {
        ...
        for (RepoDatabase upgradeItem : repoDatabase) {
            ...
            String databaseLatestRelease = upgradeItem.getLatestRelease();
            switch (api) {
                case "github":
                    ...
                    try {
                        ...
                        latestRelease = latestReleaseJson.getString("name");
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                    if (!lastTag.equals(databaseLatestTag) || !latestRelease.equals(databaseLatestRelease)) {
                        RepoDatabase repoToUpdate = LitePal.find(RepoDatabase.class, id);
                        repoToUpdate.setLatestTag(lastTag);
                        repoToUpdate.setLatestRelease(latestRelease);
