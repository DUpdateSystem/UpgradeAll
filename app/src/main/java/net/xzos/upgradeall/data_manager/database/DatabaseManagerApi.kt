...
object DatabaseManagerApi : DatabaseApi {
    ...
    private val nativeAppDatabase: List<RepoDatabase> = LitePal.findAll()
    private val nativeHubDatabase: List<net.xzos.upgradeall.data_manager.database.litepal.HubDatabase> = LitePal.findAll()

    override val appDatabases: List<AppDatabase> = nativeAppDatabase.map {
        if (it.type == null) {
    ...
    override val hubDatabases: List<HubDatabase> = nativeHubDatabase.map {
