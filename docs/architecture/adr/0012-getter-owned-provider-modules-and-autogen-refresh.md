...
This is a fixture/cache-backed generation seam, not live GitHub transport. Product runtime update-checks for generated GitHub packages use the provider cache through `getter.provider.github.release_candidates`; Flutter/native product requests stay package/update oriented and do not carry GitHub fixture bodies, API base URLs, endpoint controls, raw provider payloads, cache modes, or Lua source. Live HTTP/auth/rate-limit handling, global GitHub repository search, latest-commit live package generation, downloads, and installer semantics remain later slices.
