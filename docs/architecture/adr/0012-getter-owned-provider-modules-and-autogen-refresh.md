...
1. **Catalog/index refresh/cache**: fetch or revalidate F-Droid endpoint facts and store provider/source cache in `cache.db` with freshness metadata.
...
- generated F-Droid package evaluation through `luaclass.fdroid_android` using only `package_name` in the common case;
