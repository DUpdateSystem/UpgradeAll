...
getter --data-dir <path> autogen fdroid preview --index <index.xml> [--package <package-name>...] [--inventory <installed.json>]
getter --data-dir <path> autogen fdroid apply --preview <preview.json> (--accept-all|--accept <package-id>...)
getter --data-dir <path> autogen github preview --owner <owner> --repo <repo> --android-package <package-name> --releases <fixture.json> [--display-name <name>] [--asset-include <regex>] [--asset-exclude <regex>] [--include-prereleases]
getter --data-dir <path> autogen github apply --preview <preview.json> (--accept-all|--accept <package-id>...)
getter --data-dir <path> provider github releases --owner <owner> --repo <repo> [--releases <fixture.json>] [--asset-include <regex>] [--asset-exclude <regex>] [--include-prereleases] [--refresh]
getter --data-dir <path> provider github latest-commit --owner <owner> --repo <repo> --commit <fixture.json> [--ref <ref>] [--refresh]
...
The first F-Droid autogen CLI/dev slice is fixture-backed and offline: `autogen fdroid preview --index <index.xml> --package <package-name>` parses the supplied F-Droid index through getter-owned provider/cache code, maps requested upstream package names to package paths such as `android/f-droid/app/org.fdroid.fdroid`, skips packages covered by higher-priority registered repositories, and returns a generated package-directory preview without writing files. The same command may use `--inventory <installed.json>` to match installed Android package facts from an installed-inventory fixture against the cached F-Droid catalog before producing the same preview shape; at least one `--package` or `--inventory` input is required, and both may be supplied. `autogen fdroid apply` writes accepted package directories to the configured generated repository and records `.autogen.jsonc` with generator `fdroid-catalog`. Generated F-Droid package output uses `luaclass.fdroid_android` and is validated through the provider-backed runtime update-check path, not plain package evaluation. This slice does not perform live HTTP, downloads, installer handoff, Flutter/Kotlin provider parsing, or custom F-Droid endpoint generation; those remain governed by ADR-0012 and later provider/downloader/installer slices.

The GitHub releases provider CLI/dev slice is getter-backed: `provider github releases --owner <owner> --repo <repo> [--releases <fixture.json>]` reads or refreshes the per-project GitHub releases provider cache, normalizes matching release assets into update candidates, and returns diagnostics such as `provider.github.asset_not_found` when filters match no assets. Supplying `--releases <fixture.json>` uses a controlled GitHub REST releases JSON snapshot for deterministic tests/development; omitting it uses getter-owned live GitHub REST transport against the configured/default API base. `--asset-include` and `--asset-exclude` are regex filters over asset names, prereleases are excluded unless `--include-prereleases` is supplied, and `--refresh` exercises forced provider-cache replacement/stale semantics. The command does not generate packages, run downloads, invoke installers, create runtime actions, or move any GitHub parsing/filtering/transport/cache policy into Flutter/Kotlin.

The first GitHub Android APK autogen CLI/dev slice is fixture-backed and offline: `autogen github preview --owner <owner> --repo <repo> --android-package <package-name> --releases <fixture.json>` refreshes/reads the GitHub release provider cache from a committed or controlled REST releases snapshot, verifies that the configured release/asset filters produce at least one candidate, maps the typed project coordinates to a deterministic package id such as `android/github/DUpdateSystem/UpgradeAll/net.xzos.upgradeall`, and returns a generated package-directory preview without writing files. The preview writes only getter-owned source artifacts when later applied: `metadata.jsonc`, `Manifest` with GitHub source-response SHA-512 provenance, `9999.lua` using `luaclass.github_android_apk`, and package-local `.autogen.jsonc` ownership with generator `github-releases`. `--asset-include` and `--asset-exclude` are passed into the generated Lua package as asset-name regex filters; prereleases are excluded unless `--include-prereleases` is supplied. `autogen github apply` writes accepted package directories to the configured generated repository and preserves tracked user state through the common autogen apply path. Generated GitHub packages are validated through the provider-backed runtime update-check path after the provider cache is populated from the same snapshot; plain package evaluation is not the validation path. This slice does not perform live GitHub HTTP/auth/rate-limit handling, global GitHub search/catalog generation, latest-commit live semantics, downloads, installer handoff, Flutter/Kotlin provider parsing, or Flutter/Kotlin package-path/Lua generation.

The fixture-backed GitHub latest-commit CLI/dev slice is `provider github latest-commit --owner <owner> --repo <repo> --commit <fixture.json> [--ref <ref>] [--refresh]`. It parses a controlled GitHub commit JSON response through getter-owned provider/cache code and returns `operation = "github.latest_commit"`, `live = true`, `version`, `revision`, and `latest_commit` as a live/floating revision DTO. The omitted `--ref` default is the literal cache-key/query ref `HEAD`; explicit refs produce distinct cache keys. Latest-commit provider cache keys are distinct from release cache keys and include a dedicated latest-commit provider/cache version, request type, API-base digest, owner, repo, and ref. Empty or whitespace commit SHAs are provider refresh failures and are never cached; during forced refresh with an existing valid cache entry, they return stale cache diagnostics. The response is not an ordinary update-check result: it does not contain `candidates`, `selected_update`, artifact lists, runtime actions, or `action_id`. Malformed fixture JSON and invalid commit facts use provider operation exit code `30`; missing or unsupported CLI flags use usage exit code `2`. This slice does not generate packages, run Lua provider modules, perform live HTTP/auth/rate-limit handling, create runtime actions, download files, invoke installers, or move any GitHub parsing/filtering into Flutter/Kotlin.
