...
private val databaseChangeAnnotation =
    net.xzos.upgradeall.core.system_api.annotations.DatabaseApi.databaseChanged::class.java

object DatabaseApi : RegisterApi(databaseChangeAnnotation) {
    ...
    private fun notify(database: Any) {
        if ((database is AppDatabase)
            || database is HubDatabase
        ) {
            runNoReturnFun(databaseChangeAnnotation, database)
        }
    }
}
