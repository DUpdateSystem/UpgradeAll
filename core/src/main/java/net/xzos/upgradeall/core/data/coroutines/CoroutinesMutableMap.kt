...
class CoroutinesMutableMap<K, V>(hash: Boolean = false, map: Map<K, V>? = null) : MutableMap<K, V> {
    ...
    override val entries: MutableSet<MutableMap.MutableEntry<K, V>>
        get() = mutex.runWithLock {
            mutableMap.entries.toMutableSet()
        }
    override val keys: MutableSet<K>
        get() = mutex.runWithLock {
            mutableMap.keys.toMutableSet()
        }
    override val values: MutableCollection<V>
        get() = mutex.runWithLock {
            mutableMap.values.toMutableSet()
        }
...
fun <E, V> Map<E, V>.toCoroutinesMutableMap(hash: Boolean = false) = CoroutinesMutableMap(hash, this)
