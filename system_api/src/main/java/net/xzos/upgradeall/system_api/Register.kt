...
open class Register(private val mainAnnotationClass: Class<out Annotation>,
                    private val annotationClassList: List<Class<out Annotation>>
) {
    private val methodMap: HashMap<Class<out Annotation>, MutableList<Pair<Any, Method>>> = hashMapOf()

    fun register(any: Any) {
        val classObj = any::class.java
        if (classObj.isAnnotationPresent(mainAnnotationClass)) {
            val method: Array<Method> = classObj.declaredMethods
            for (m in method) {
                for (annotationClass in annotationClassList) {
                    if (m.isAnnotationPresent(annotationClass)) {
                        m.isAccessible = true
                        (methodMap[annotationClass] ?: mutableListOf<Pair<Any, Method>>().apply {
