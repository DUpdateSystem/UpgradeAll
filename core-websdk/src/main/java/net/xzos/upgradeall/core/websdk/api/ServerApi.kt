...
class ServerApi internal constructor(
    ...
    private fun getAppUpdateCache(data: MultiRequestData): Pair<Map<AppData, ReleaseGson?>, MultiRequestData> {
        ...
        data.appList.forEach {
            ...
            val cache =
                dataCache.get(SaveMode.DISK_ONLY, key, AppReleaseListEncoder)
    ...
    override fun getAppReleaseList(data: SingleRequestData): List<ReleaseGson>? {
        ...
        return lockMap.runWith(key) {
            dataCache
                .get(it, SaveMode.DISK_ONLY, key, AppReleaseListEncoder) {
                    clientProxyApi.getAppReleaseList(data)
                }?.apply {
    ...
    override fun getDownloadInfo(
        ...
        return downloadItemList.ifEmpty {
            val releaseList = getAppReleaseList(data)

            val asset =
                releaseList
                    ?.getOrNull(assetIndex.first)
                    ?.assetGsonList
                    ?.getOrNull(assetIndex.second) ?: return emptyList()
            listOf(
                DownloadItem(
                    asset.fileName,
                    asset.downloadUrl ?: return emptyList(),
                    emptyMap(),
                    emptyMap(),
                ),
            )
